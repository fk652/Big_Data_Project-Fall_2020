...
if __name__ == "__main__":
    ...
    stock_info_data = spark.read.format('csv') \
                    .options(header='true', inferschema='false') \
                    .load(sys.argv[3])
    stock_info_data = stock_info_data.select('Symbol',
                                            'GICS Sector',
                                            'GICS Sub-Industry')
    ...
    joined_data = joined_data.join(stock_info_data,  on=['Symbol'], how='inner')
    ...
    joined_data.write.options(emptyValue='').csv('S&P_500_Join_Oxford_JohnHopkins_News.out')
