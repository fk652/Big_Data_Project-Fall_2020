...
if __name__ == "__main__":
    ...
    spark = SparkSession.builder.appName('sp500_calculations').getOrCreate()
    ...
    # cumulative log return sum per company
    ...
    # total log return sum per company
    ...
    sp_sum = sp_sum.join(sector_info, on='Symbol', how='inner')
    ...
    # top 10 and bottom 10 companies
    top_10_sum = sp_sum.orderBy(col('ReturnSum').desc()).limit(10)
    bottom_10_sum = sp_sum.orderBy(col('ReturnSum').asc()).limit(10)
    ...
    # two versions of final sums, 1 file sorted by rank, 1 file sorted by symbol
    ...
    sp_sum_symbol_order = sp_sum_rank_order.orderBy(col('Symbol'))
    ...
    sp_sum_rank_order.write.options(emptyValue='').csv('S&P_500_Log_Sums_Rank.out')

    header = [tuple(sp_sum_symbol_order.columns)]
    header = spark.createDataFrame(header)
    sp_sum_symbol_order = header.union(sp_sum_symbol_order)
    sp_sum_symbol_order.write.options(emptyValue='').csv('S&P_500_Log_Sums_Symbol.out')

    header = [tuple(cum_sum.columns)]
    header = spark.createDataFrame(header)
    cum_sum = header.union(cum_sum)
    cum_sum.write.options(timestampFormat='yyyy-MM-dd', emptyValue='').csv('S&P_500_Cum_Sums.out')

    header = [tuple(top_10_sum.columns)]
    header = spark.createDataFrame(header)
    top_10_sum = header.union(top_10_sum)
    top_10_sum.write.options(emptyValue='').csv('S&P_500_Top_10.out')

    header = [tuple(bottom_10_sum.columns)]
    header = spark.createDataFrame(header)
    bottom_10_sum = header.union(bottom_10_sum)
    bottom_10_sum.write.options(emptyValue='').csv('S&P_500_Bottom_10.out')

    header = [tuple(top_5_drops.columns)]
    header = spark.createDataFrame(header)
    top_5_drops = header.union(top_5_drops)
    top_5_drops.write.options(timestampFormat='yyyy-MM-dd', emptyValue='').csv('S&P_500_Top_5_Drops.out')

    header = [tuple(top_5_gains.columns)]
    header = spark.createDataFrame(header)
    top_5_gains = header.union(top_5_gains)
    top_5_gains.write.options(timestampFormat='yyyy-MM-dd', emptyValue='').csv('S&P_500_Top_5_Gains.out')
